"""
Error analysis for the 3-class causal relation extraction on CNC dataset.
...
from shared.data_loader import load_cnc_data

TEST_FILE   = os.path.join(ROOT, "data", "CausalNewsCorpus", "dev_clean.txt")
...
def load_predictions(model_name: str):
    path = os.path.join(RESULTS_DIR, model_name, "cnc", "predictions.txt")
    with open(path, encoding="utf-8") as f:
        return [line.strip() for line in f]

test_examples = load_cnc_data(TEST_FILE)
...
def bucket(length):
    if length <= 27:  return "short  (<=27)"
    if length <= 38:  return "medium (28-38)"
    return                    "long   (>38)"
...
for bname in ["short  (<=27)", "medium (28-38)", "long   (>38)"]:
