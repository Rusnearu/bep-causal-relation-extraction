...
from src.shared.evaluation import evaluate as shared_evaluate

parser = argparse.ArgumentParser()
parser.add_argument('--data_dir', type=str, default='data/semeval2010/cgcn')
parser.add_argument('--vocab_dir', type=str, default='data/semeval2010/cgcn/vocab')
parser.add_argument('--eval_dir', type=str, default='results/cgcn/semeval2010', help='Directory to save evaluation results.')
...
file_logger = helper.FileLogger(model_save_dir + '/' + opt['log'], header="# epoch\ttrain_loss\ttest_loss\ttest_score\tbest_test_score")
...
for epoch in range(1, opt['num_epoch'] + 1):
    ...
    print("epoch {}: train_loss = {:.6f}, test_loss = {:.6f}, test_f1 = {:.4f}".format(epoch,
        train_loss, test_loss, test_f1))
    test_score = test_f1
    file_logger.log("{}\t{:.6f}\t{:.6f}\t{:.4f}\t{:.4f}".format(epoch, train_loss, test_loss, test_score, max([test_score] + test_score_history)))

    # save
    model_file = model_save_dir + '/checkpoint_epoch_{}.pt'.format(epoch)
    trainer.save(model_file, epoch)
    if epoch == 1 or test_score > max(test_score_history):
        copyfile(model_file, model_save_dir + '/best_model.pt')
        best_predictions = predictions[:]
        print("new best model saved.")
        file_logger.log("new best model saved at epoch {}: {:.2f}\t{:.2f}\t{:.2f}"
            .format(epoch, test_p * 100, test_r * 100, test_score * 100))
    if epoch % opt['save_epoch'] != 0:
        os.remove(model_file)

    # lr schedule
    if len(test_score_history) > opt['decay_epoch'] and test_score <= test_score_history[-1] and \
            opt['optim'] in ['sgd', 'adagrad', 'adadelta']:
        current_lr *= opt['lr_decay']
        trainer.update_lr(current_lr)

    test_score_history += [test_score]
    print("")

print("Training ended with {} epochs.".format(epoch))

# final evaluation using shared framework
print("\nRunning final evaluation with shared evaluator...")
gold_labels = test_batch.gold()
os.makedirs(opt['eval_dir'], exist_ok=True)

# save predictions
pred_file = os.path.join(opt['eval_dir'], 'predictions.txt')
with open(pred_file, 'w') as f:
    for pred in best_predictions:
        f.write(pred + '\n')
print("Best predictions saved to {}".format(pred_file))
